    ...
    public RouteLocator gatewayRouter(RouteLocatorBuilder builder){
        ...
        return builder.routes()
                ...
                .route(p -> p.path("/api/phases/**")
                        .filters(f -> f.rewritePath("/api/phases/(?<segment>.*)", "/api/projects/${segment}"))
                        .uri("lb://project-service"))
                .route(p -> p.path("/api/tasks/**")
                        .filters(f -> f.rewritePath("/api/tasks/(?<segment>.*)", "/api/projects/${segment}"))
                        .uri("lb://project-service"))
                .route(p -> p.path("/api/etapes/**")
                        .filters(f -> f.rewritePath("/api/etapes/(?<segment>.*)", "/api/projects/${segment}"))
                        .uri("lb://project-service"))
